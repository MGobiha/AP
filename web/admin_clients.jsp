...
<%
  List<Users> clients = (List<Users>) request.getAttribute("clients");
...
<style>
  ...
  .form-grid{display:grid;grid-template-columns:1.2fr .8fr 1.2fr .8fr .8fr .9fr auto;gap:10px;padding:14px 16px;}
  ...
  .tag.vip{background:#fff7ed;color:#c2410c;border-color:#fed7aa;}
  .tag.client{background:#f1f5f9;color:#334155;border-color:#e2e8f0;}
  ...
  <table>
    ...
    %>
      ...
      <tr>
        ...
        <td><%=u.getUsername()%></td>
        <td>
          <span class="tag <%= "VIP_CLIENT".equalsIgnoreCase(u.getRole()) ? "vip" : "client" %>">
            <%=u.getRole()%>
          </span>
        </td>
        <td>
          <div class="actions">
            ...
            <a class="icon-btn edit"
               href="<%=request.getContextPath()%>/admin?page=clients&editId=<%=u.getId()%>"
               title="Edit">
      ...
      <tr>
        ...
        <td colspan="6" style="padding:0;">
          <form action="<%=request.getContextPath()%>/admin" method="post"
                style="display:grid;grid-template-columns:1fr 130px 1fr 160px 140px 150px;gap:10px;padding:12px 14px;">
            ...
            <select class="cell-input" name="role">
              <option value="CLIENT" <%= "CLIENT".equalsIgnoreCase(u.getRole())?"selected":"" %>>CLIENT</option>
              <option value="VIP_CLIENT" <%= "VIP_CLIENT".equalsIgnoreCase(u.getRole())?"selected":"" %>>VIP_CLIENT</option>
            </select>
            ...
            <div class="actions">
              ...
              <a class="icon-btn danger" title="Cancel"
                 href="<%=request.getContextPath()%>/admin?page=clients">
                <svg class="icon" viewBox="0 0 24 24">
                  <path d="M18 6L6 18"/>
                  <path d="M6 6l12 12"/>
                </svg>
